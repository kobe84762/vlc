...
{
    writeToLog("bool ISegment::prepareChunk(SharedResources *res, SegmentChunk *chunk, BaseRepresentation *rep) called.");
    CommonEncryption enc = encryption;
    enc.mergeWith(rep->intheritEncryption());

    writeToLog("bool ISegment::prepareChunk(SharedResources *res, SegmentChunk *chunk, BaseRepresentation *rep) Key ID: " + std::string(enc.iv.begin(), enc.iv.end()));
